// import { io } from '../../app';
// import { createUsersRoomGetter } from '../utils';
// import { MySocket, ClientToServerEvents, ServerToClientEvents } from '../../types/types';
// import { MySocket } from 'src/types/types';

export function subscribeToBatleshipsEvents() {}
// export function subscribeToBatleshipsEvents(socket: MySocket) {
//     const getUsersRoom = createUsersRoomGetter(socket);
// }

export function unsubscribeFromBattleships() {}
// export function unsubscribeFromBattleships(socket: MySocket) {
//     console.log(socket);
// }
